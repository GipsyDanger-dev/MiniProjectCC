...
const charts = [
    { key: "gas", label: "Gas (PPM)", stroke: "#c4b5fd", g: "gasG", threshold: 500 },
    { key: "smoke", label: "Smoke (PPM)", stroke: "#a0c3ec", g: "smokeG", threshold: 250 },
    { key: "temp", label: "Temp (°C)", stroke: "#ff7a17", g: "tempG", threshold: 45 },
    { key: "flame", label: "Flame (%)", stroke: "#ffffff", g: "flameG", threshold: 20 },
];
...
export default function SensorData({ activeRoom, iot }) {
    ...
    return (
        <div className="pb-4 sm:pb-5 space-y-3 sm:space-y-5">
            ...
            <MagicBentoGrid enableSpotlight enableBorderGlow enableTilt enableMagnetism clickEffect glowColor="255, 255, 255" spotlightRadius={300} className="grid gap-2 sm:gap-3 grid-cols-2 lg:grid-cols-4">
                {cards.map(c => (
                    <GlassSurface key={c.title} className="p-3 sm:p-4 bento-card bento-card--border-glow group">
                        ...
                        <span className="text-[11px] uppercase tracking-[1.4px] text-[#7d8187] font-normal relative z-10" style={{ fontFamily: "'Geist Mono', monospace" }}>{c.title}</span>
                        <div className="mt-2 flex items-end justify-between relative z-10">
                            <p className="text-3xl font-normal tracking-tight">{c.value}{c.unit && <span className="text-sm text-[#7d8187] ml-1">{c.unit}</span>}</p>
                            <span className="text-[11px] text-[#c4b5fd] font-normal" style={{ fontFamily: "'Geist Mono', monospace", letterSpacing: '0.5px' }}>{c.delta}</span>
                        </div>
                    </GlassSurface>
                ))}
            </MagicBentoGrid>
            ...
            <GlassSurface className="p-3 sm:p-5">
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
                    ...
                    <div className="inline-flex rounded-lg bg-[#0a0a0a] border border-[#212327] p-0.5 text-xs self-start">{["1H", "6H", "24H", "7D"].map(t => <button key={t} onClick={() => setRange(t)} className={`px-2 sm:px-2.5 py-1 rounded-md transition-all duration-150 font-normal text-[11px] sm:text-xs ${range === t ? "bg-white text-[#0a0a0a]" : "text-[#7d8187] hover:text-white"}`}>{t}</button>)}</div>
                </div>
                <div className="grid gap-2 sm:gap-3 grid-cols-1 sm:grid-cols-2">
                    {charts.map(s => (
                        <div key={s.key} className="h-[200px] rounded-lg border border-[#212327] bg-[#0a0a0a] p-2 hover:border-[rgba(124,58,237,0.2)] transition-colors duration-200">
                            <div className="px-2 pt-1 text-[11px] font-normal text-[#7d8187]">{s.label}</div>
                            <ResponsiveContainer width="100%" height="90%">
                                <AreaChart data={data} margin={{ top: 10, right: 8, left: -12, bottom: 0 }}>
                                    <defs><linearGradient id={s.g} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={s.stroke} stopOpacity={0.2} /><stop offset="100%" stopColor={s.stroke} stopOpacity={0.01} /></linearGradient></defs>
                                    <CartesianGrid strokeDasharray="4 6" stroke="#212327" vertical={false} />
                                    <XAxis dataKey="time" tick={{ fill: "#7d8187", fontSize: 10 }} axisLine={false} tickLine={false} minTickGap={20} />
                                    <YAxis tick={{ fill: "#7d8187", fontSize: 10 }} axisLine={false} tickLine={false} width={28} />
                                    <Tooltip contentStyle={{ background: "#191919", border: "1px solid rgba(124,58,237,0.2)", borderRadius: "8px", color: "#fff", fontSize: 12 }} />
                                    <ReferenceLine y={s.threshold} stroke="rgba(239,68,68,0.5)" strokeDasharray="6 6" />
                                    <Area type="monotone" dataKey={s.key} stroke={s.stroke} strokeWidth={1.5} fill={`url(#${s.g})`} dot={false} activeDot={{ r: 3, fill: s.stroke }} />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    ))}
            ...
            <GlassSurface className="p-3 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-4">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                        <div className="h-7 sm:h-8 rounded-[9999px] border border-[#212327] bg-[#0a0a0a] px-2 sm:px-2.5 flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs text-[#7d8187]"><Filter className="w-3 h-3 sm:w-3.5 sm:h-3.5" />Filter</div>
                ...
                <div className="overflow-auto thin-scroll">
                    <table className="w-full min-w-[700px] text-sm">
                        <thead><tr className="text-[11px] uppercase tracking-[1.4px] text-[#7d8187] border-b border-[#212327] font-normal"><th className="text-left py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Timestamp</th><th className="text-left py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Gas</th><th className="text-left py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Smoke</th><th className="text-left py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Temp</th><th className="text-left py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Flame</th><th className="text-right py-2" style={{ fontFamily: "'Geist Mono', monospace" }}>Status</th></tr></thead>
                        <tbody>{rows.map((r, i) => <tr key={`${r[0]}-${i}`} className={`border-b border-[#212327] hover:bg-[#1a1c20] ${i % 2 === 0 ? "bg-[#0a0a0a]" : ""}`}><td className="py-2 mono-value" style={{color:'#7d8187'}}>{r[0]}</td><td className="mono-value">{r[1]}</td><td className="mono-value">{r[2]}</td><td className="mono-value">{r[3]}</td><td className="mono-value">{r[4]}</td><td className="text-right"><span className={`inline-flex px-1.5 py-0.5 rounded-[9999px] text-[10px] border font-normal ${r[5] === "BAHAYA" ? "bg-[#ef4444]/10 text-[#ef4444] border-[#ef4444]/30" : "bg-[#22c55e]/10 text-[#22c55e] border-[#22c55e]/30"}`}>{r[5]}</span></td></tr>)}</tbody>
                    </table>
                </div>
            </GlassSurface>
        </div>

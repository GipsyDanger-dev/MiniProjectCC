...
export default function StatCard({ title, value, sub, icon: Icon, badge, accent = "white", danger = false }) {
    return (
        <GlassSurface className={cn("p-3 sm:p-4 bento-card bento-card--border-glow group", danger && "animate-danger-border border-[#ef4444]/50")}>
            ...
            <div className="flex items-center justify-between relative z-10">
                <p className="text-[10px] sm:text-[11px] uppercase tracking-[1.2px] sm:tracking-[1.4px] font-normal text-[#7d8187]" style={{ fontFamily: "'Geist Mono', monospace" }}>{title}</p>
                {Icon && (
                    <span className={cn(
                        "w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center transition-all duration-200",
                        danger ? "bg-[#ef4444]/15 text-[#ef4444] animate-danger-shake" : "bg-[#1a1c20] text-white border border-[#212327] group-hover:border-[rgba(124,58,237,0.3)] group-hover:text-[#c4b5fd]"
                    )}>
